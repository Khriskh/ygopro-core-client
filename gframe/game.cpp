...
bool Game::Initialize() {
	...
	lifepointsFont2 = irr::gui::CGUITTFont::createTTFont(env, gameConf.numfont6, 20);
	...
	//main menu
	wchar_t strbuf[256];
	myswprintf(strbuf, L"YGOProES %X.0%X.%X ChristianYGO", PRO_VERSION >> 12, (PRO_VERSION >> 4) & 0xff, PRO_VERSION & 0xf);
...
//	btnTestMode = env->addButton(rect<s32>(10, 135, 270, 165), wMainMenu, BUTTON_TEST_MODE, dataManager.GetSysString(1203));
	...
	stRanked = env->addStaticText(L"", rect<s32>(120, 35, 200, 55), false, false, wHostPrepare);
	for(int i = 0; i < 2; ++i) {
	...
	//replay control
	wReplayControl = env->addStaticText(L"", rect<s32>(205, 118, 295, 273), false, true, 0, -1, true);
	wReplayControl->setBackgroundColor(0x00FFFFFF);
	wReplayControl->setVisible(false);
	btnReplayStart = env->addButton(rect<s32>(5, 5, 85, 25), wReplayControl, BUTTON_REPLAY_START, L"");
	btnReplayStart->setImage(imageManager.tBtnr1);
	btnReplayStart->setScaleImage(true);
	btnReplayStart->setUseAlphaChannel(true);
	btnReplayStart->setDrawBorder(false);
	btnReplayPause = env->addButton(rect<s32>(5, 30, 85, 50), wReplayControl, BUTTON_REPLAY_PAUSE, L"");
	btnReplayPause->setImage(imageManager.tBtnr2);
	btnReplayPause->setScaleImage(true);
	btnReplayPause->setUseAlphaChannel(true);
	btnReplayPause->setDrawBorder(false);
	btnReplayStep = env->addButton(rect<s32>(5, 55, 85, 75), wReplayControl, BUTTON_REPLAY_STEP, L"");
	btnReplayStep->setImage(imageManager.tBtnr3);
	btnReplayStep->setScaleImage(true);
	btnReplayStep->setUseAlphaChannel(true);
	btnReplayStep->setDrawBorder(false);
	btnReplayUndo = env->addButton(rect<s32>(5, 80, 85, 100), wReplayControl, BUTTON_REPLAY_UNDO, L"");
	btnReplayUndo->setImage(imageManager.tBtnr4);
	btnReplayUndo->setScaleImage(true);
	btnReplayUndo->setUseAlphaChannel(true);
	btnReplayUndo->setDrawBorder(false);
	btnReplaySwap = env->addButton(rect<s32>(5, 105, 85, 125), wReplayControl, BUTTON_REPLAY_SWAP, L"");
	btnReplaySwap->setImage(imageManager.tBtnr5);
	btnReplaySwap->setScaleImage(true);
	btnReplaySwap->setUseAlphaChannel(true);
	btnReplaySwap->setDrawBorder(false);
	btnReplayExit = env->addButton(rect<s32>(5, 130, 85, 150), wReplayControl, BUTTON_REPLAY_EXIT, L"");
	btnReplayExit->setImage(imageManager.tBtnr6);
	btnReplayExit->setScaleImage(true);
	btnReplayExit->setUseAlphaChannel(true);
	btnReplayExit->setDrawBorder(false);
	//chat
...
void Game::OnResize() {
	...
	lifepointsFont2 = irr::gui::CGUITTFont::createTTFont(env, gameConf.numfont6, 20 * yScale);
	...
	wLanWindow->setRelativePosition(ResizeWin(220, 100, 800, 535));
