...
void Game::DrawMisc() {
	...
	if((dInfo.turn % 2 && dInfo.isFirst) || (!(dInfo.turn % 2) && !dInfo.isFirst)) {
		...
		driver->draw2DImage(imageManager.tAvatar2, Resize(310, 5, 630, 105), recti(0, 0, 600, 150), 0, 0, true);
		driver->draw2DImage(imageManager.tAvatar1, Resize(691, 5, 1011, 105), recti(0, 0, 600, 150), 0, 0, true);
	}
	...
	recti p1size = Resize(370, 45, 629, 50);
	recti p2size = Resize(986, 31, 986, 50);
	if(!dInfo.isTag || !dInfo.tag_player[0])
		nameFont->draw(dInfo.hostname, p1size, 0xffffffff, false, false, 0);
	else
		nameFont->draw(dInfo.hostname_tag, p1size, 0xffffffff, false, false, 0);
	if(!dInfo.isTag || !dInfo.tag_player[1]) {
		auto cld = textFont->getDimension(dInfo.clientname);
		p2size.UpperLeftCorner.X -= cld.Width;
		nameFont->draw(dInfo.clientname, p2size, 0xffffffff, false, false, 0);
	} else {
		auto cld = textFont->getDimension(dInfo.clientname_tag);
		p2size.UpperLeftCorner.X -= cld.Width;
		nameFont->draw(dInfo.clientname_tag, p2size, 0xffffffff, false, false, 0);
	}
