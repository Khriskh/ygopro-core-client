...
void Game::DrawMisc() {
	...
	if((dInfo.turn % 2 && dInfo.isFirst) || (!(dInfo.turn % 2) && !dInfo.isFirst)) {
		...
		driver->draw2DImage(imageManager.tAvatar1, Resize(310, 5, 630, 105), recti(0, 0, 600, 115), 0, 0, true);
		driver->draw2DImage(imageManager.tAvatar2, Resize(691, 5, 1011, 105), recti(0, 0, 600, 115), 0, 0, true);
	} else {
		//driver->draw2DRectangle(0xa0000000, Resize(689, 8, 991, 51));
		//driver->draw2DRectangleOutline(Resize(689, 8, 991, 51), 0xffff8080);
		
		driver->draw2DImage(imageManager.tAvatar2, Resize(310, 5, 630, 105), recti(0, 0, 600, 115), 0, 0, true);
		driver->draw2DImage(imageManager.tAvatar1, Resize(691, 5, 1011, 105), recti(0, 0, 600, 115), 0, 0, true);
	}
